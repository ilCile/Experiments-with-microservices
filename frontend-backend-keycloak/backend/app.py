...
def verify_token(token):
    try:
        ...
        decoded = jwt.decode(
            ...
            signing_key,
            algorithms=["RS256"],
            options={"verify_iss": False} #l'issuer nel token risulta localhost:8080 ma il backend lo vede come keycloak:8080
        )
        return decoded

    except jwt.ExpiredSignatureError:
        print("Token scaduto", flush=True)
        return None
    except jwt.InvalidTokenError as e:
        print(f"Token non valido: {e}", flush=True)
        return None
    except Exception as e:
        print(f"Errore in verify_token: {e}", flush=True)
        return None
